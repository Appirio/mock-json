/**
 * This file is a modified version of the MockJSON "jquery.mockjson.js" file that I downloaded
 * from: https://github.com/mennovanslooten/mockJSON.
 *
 * I use this file mainly for MockJSON's innovative template system, NOT for its AJAX request
 * interception, which I perform using the Sinon.js "fake server" instead.
 *
 * The following is the list of changes that I made to this file to (1) prevent MockJSON from
 * intercepting AJAX requests, (2) expose MockJSON's template system for use outside of this file,
 * and (3) completely remove MockJSON's dependency on jQuery.
 *
 * -- I commented out the code that defined the "_mocked" array and the "$.mockJSON" function.
 * -- I commented out the code that overrode/replaced the jQuery "$.ajax" function.
 * -- I declared a new local variable called "mockJSON" and initialized it to an empty object.
 * -- I replaced all instances of "$.mockJSON" with the local variable "mockJSON".
 * -- One of the functions originally used jQuery's $.isArray() function, so I changed it to use
 *    the native ECMAScript-5 Array.isArray() function instead and added polyfill code to ensure
 *    that it is available. To see the relevant code, perform a text search for "ChrisChan" below.
 * -- I removed the import of "jQuery" into the module as "$" (i.e., I removed "jQuery" from the
 *    argument list of the module's enclosing immediately-invoked function expression and removed
 *    "$" from the parameter list).
 * -- I exported the "mockJSON" local variable from the module as "MockJsonTemplateModule"
 *    (i.e., I return "mockJSON" at the end of the module, which gets assigned to the
 *    "MockJsonTemplateModule" variable at the top of the module).
 * -- I added some missing semicolons to appease the Eclipse warnings that were showing up... =)
 *
 * As a result, we can call
 *
 *     MockJsonTemplateModule.generateFromTemplate( template );
 *
 * to generate random JSON based on the specified template. (What actually gets returned is a
 * JavaScript object based on the template, NOT a JSON string, but we can easily pass it to
 * JSON.stringify() if needed.) We can even use the EXACT same template annotations used by
 * MockJSON: http://experiments.mennovanslooten.nl/2010/mockjson/. Moreover, we can add custom
 * MockJSON template keywords (as described in the link) in a similar manner:
 *
 *     MockJsonTemplateModule.data.US_STATE = [
 *         'Alabama', 'Alaska', ... , 'Wisconsin', 'Wyoming'
 *     ];
 *
 * Also, like the original MockJSON, we can generate "predictable, reproducible JSON" by setting:
 *
 *     MockJsonTemplateModule.random = false;
 *
 * NOTE: Apparently, setting "random" to false causes MockJSON to generate several random JSON
 * strings based on a given template, and loop through those same JSON strings for each call to
 * generateFromTemplate(template). This is normal behavior from the original MockJSON library.
 */
var MockJsonTemplateModule = (function () {
  ...
  mockJSON.generateFromTemplate = function (template, name) {
    ...
    switch (type(template)) {
      case 'array':
        ...
        //edit: Will S. - don't eat my normal array!
        if (length == 0) {
          generated = template;
        }
...
// ChrisChan: The Array.isArray() function is part of ECMAScript 5, but just in case it isn't
// available, the following polyfill code makes sure that it is. The polyfill code is from:
...
// ChrisChan: I changed this function to use Array.isArray() instead of jQuery's $.isArray().
  function type(obj) {
