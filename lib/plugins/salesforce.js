...
var MockJsonUtilsForSFDC = {
    ...
    'getWhereClauseString': function (queryStr) {
        var whereClause;
        if (queryStr.lastIndexOf('+where+') > -1) {
            whereClause = queryStr.split("+where+")[ 1 ];
        }
        return whereClause;
    },
    ...
    'getWhereClauseObject': function (whereStr) {
        ...
        if (whereStr != null) {
            whereObject = {};
            var whereArray = whereStr.split("+=+");
            var length = whereArray.length;
            var fieldName = null;
            for (var i = 0; i < length; i = i + 2) {
                fieldName = whereArray[i];
                fieldValue = whereArray[i + 1];
    ...
    'getFromClauseString': function (queryStr) {
        var fromClause;
        var fromIndex = queryStr.lastIndexOf('+from+');
        if (fromIndex > -1) {
            var subFrom = queryStr.substr(fromIndex);
            fromClause = subFrom.split("+")[ 2 ];
        }
        return fromClause;
    },
    ...
    'objectsReturned': function (request, objectName, filterId) {
        var newLoc = MockJsonTemplateModule.firebaseLoc + '/' + objectName + '/';
        ...
        objectDataRef.on('value', function (dataSnapshot) {
            ...
            if (rawData) {
                ...
                for (var i = 0; i < keys.length; i++) {
                    //return only the records with the filterId if specified
                    var sfdcId = rawData[keys[i]].Id;
                    if (filterId && sfdcId != filterId) {
                        continue;
                    }
                    objsToRet.push(rawData[keys[i]]);
                }
                console.log('objsToRet', objsToRet);

                // Put the objects somewhere (anywhere) so the endpoint's handler function can access them
...
MockJsonApiLib.addMockEndpoints([
...

    {
        ...
        asyncInitializer: function (request) {

            console.log("MOCK API initializer for request: ", request);
            ...
            var whereObject = MockJsonUtilsForSFDC.getWhereClauseObject(whereClause);

            //get the FROM Object name for request generation
            var objectName = MockJsonUtilsForSFDC.getFromClauseString(queryStr);
            ...
            if (whereObject) {
                MockJsonUtilsForSFDC.objectsReturned(request, objectName, whereObject.id);
            }
            else {
                MockJsonUtilsForSFDC.objectsReturned(request, objectName);
            }
    ...
    },
    {
        ...
        asyncInitializer: function (request) {
            ...
            //convert to object
            var responseObject = JSON.parse(decodeURIComponent(respBody));
